...
- **Database**: Upstash Redis through the single client in `lib/redis.ts` (constructed from `KV_REST_API_URL`/`KV_REST_API_TOKEN`). No ORM — typed helpers in `lib/kv.ts` (board) and `lib/network.ts` (network); `lib/users.ts` lists/counts users
- **Rate limits**: `lib/ratelimit.ts` (`@upstash/ratelimit`, fail-open) — `parse` per user (30/10 min + 300/day) on every OpenAI-backed route and MCP tool, `signin` per IP in `proxy.ts`, `sheet` per user on the sheet import
- **Outbound fetches**: `lib/safe-url.ts` refuses private/loopback/link-local hosts and non-http(s) schemes; `lib/fetchPage.ts` follows redirects by hand (≤3 hops, guard re-run per hop) and caps bodies at 2 MB
